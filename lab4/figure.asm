...
# Name:     block_make
#
# Description:  Initalize the components of the block object
#       All this subroutine does it set the v. function table,
#       then call figure_make
#
# Arguments:    a0 contains the height of the figure's bounding box
#       a1 contains the width of the bounding box
#       a2 contains the address of the figure object
#
...
block_make:
    ...
    la  $t1, block_vtbl     # save the address of the table in t1
    sw  $t1, 0($a2)     # store the address of the table in the figure

    jal figure_make     # create the figure

    lw  $ra, -4+FRAMESIZE_8($sp)
    addi    $sp, $sp, FRAMESIZE_8
    jr $ra      # return from block_make
...
block_area:
    ...
    move    $t0, $a0    # move the address of the figure into t0
    lw  $s0, 4($t0)     # store the width in s0
    lw  $s1, 8($t0)     # store the height in s1

    mul $v0, $s0, $s1   # multiply the wxh to get the area -> a0
...
block_perimeter:
    ...
    move    $t0, $a0    # t0 is address of figure
    lw  $s0, 4($t0)     # s0 = width of box
    lw  $s1, 8($t0)     # s1 = height of box

    mul $t1, $s0, 2     # t1 = 2xwidth
    mul $t2, $s1, 2     # t2 = 2xheight
    add $v0, $t1, $t2   # v0 (area) = t1 + t2
	...
	addi 	$sp, $sp, FRAMESIZE_24   	
	jr 	$ra		# return from block_perimeter
...
#
    ...
    li  $v0, PRINT_INT  # set syscall to print ints
    lw  $a0, 4($s0)     # give width (the 2nd word in the figure) to syscall
    syscall
...
#
    ...
    li  $v0, PRINT_INT  # set syscall to print ints
    lw  $a0, 8($s0)     # give height (the 3rd word in the figure) to syscall
    syscall
...
#
    ...
    move    $a0, $s0    # a0 = address of figure (for call to area)
    lw  $t0, 0($s0)     # t0 = address of v-table
    lw  $t1, 0($t0)     # t1 = address of area (which one depends on v-table)
    jalr  $t1           # calculate the area and store the value in v0
    move    $a0, $v0    # a0 = area (for call to syscall)
    li  $v0, PRINT_INT  # set syscall to print ints
    syscall
...
#
    ...
    move    $a0, $s0    # a0 = address of figure (for call to perim.)
    lw  $t0, 0($s0)     # t0 = address of v-table
    lw  $t1, 4($t0)     # t1 = address of perim (which  depends on the v-table)
    jalr    $t1         # calculate the perim. and store the value in v0
    move    $a0, $v0    # a0 = perim for syscall
    li  $v0, PRINT_INT  # set syscall to print ints
    syscall
